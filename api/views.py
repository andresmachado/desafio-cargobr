...
class MapViewSet(viewsets.ModelViewSet):
    ...
    def routes(self, request, pk=None):
        """
            This endpoint calculates the best route given all parameters in request.
        """
        map_name = self.get_object()
        ...
        if origin and destiny and autonomy and lit_price:
            given_map = Map.objects.get(pk=pk)
            initial_route = given_map.routes.filter(origin=origin).annotate(Min('distance')).first()
            end_route = given_map.routes.filter(origin=initial_route.destiny)
            serializer = serializers.serialize('json', end_route)
